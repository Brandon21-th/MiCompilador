/* Generated By:JavaCC: Do not edit this line. Parser.java */
package edu.itsco;
import java.util.ArrayList;
public class Parser implements ParserConstants {
  private static ArrayList<Variable> listaVariables;

  public static void main(String args []) throws ParseException, SemanticException
  {
    listaVariables = new ArrayList<Variable>();
    Parser parser = new Parser(System.in);
    System.out.println("Bienvenidos al COMPILADOR de Brandito");
    System.out.println("-------------------------------------");
    parser.principal();
    System.out.println("Vas bien Brandito compilo exitosamente");
  }

  private static void addVariable(Variable variable) throws SemanticException{
    if(listaVariables.contains(variable)) {
      throw new SemanticException(variable, SemanticException.VARIABLE_DUPLICATED);
    }else {
      listaVariables.add(variable);

      }
  }
  private static void existeVariable(String id) throws SemanticException {
    Variable v= new Variable();
    v.setId(id);
    if(!listaVariables.contains(v)) {
      throw new SemanticException(v, SemanticException.VARIABLE_NO_DEFINED);
    }
     }

  static final public void principal() throws ParseException, SemanticException {
    jj_consume_token(MAIN);
    jj_consume_token(ID);
    label_1:
    while (true) {
      sentences();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case VAR:
      case WRITE:
      case READ:
      case IF:
      case SELECT:
      case FOR:
      case DOWHILE:
      case WHILE:
      case ID:
        ;
        break;
      default:
        jj_la1[0] = jj_gen;
        break label_1;
      }
    }
    jj_consume_token(END);
  }

  static final public void sentences() throws ParseException, SemanticException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case VAR:
      declareVariable();
      break;
    case WRITE:
      grammarwrite();
      break;
    case READ:
      grammarread();
      break;
    case ID:
      grammarAssignment();
      break;
    case IF:
      grammarIF();
      break;
    case SELECT:
      grammarSelect();
      break;
    case FOR:
      grammarFor();
      break;
    case WHILE:
      grammarWhile();
      break;
    case DOWHILE:
      grammarDowhile();
      break;
    default:
      jj_la1[1] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

  static final public void declareVariable() throws ParseException, SemanticException {
Token tipoDato;
Token id;
boolean inicializada = false;
    jj_consume_token(VAR);
    id = jj_consume_token(ID);
    tipoDato = typeDATA();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case SAME:
      jj_consume_token(SAME);
      value();
                      inicializada = true;
      break;
    default:
      jj_la1[2] = jj_gen;
      ;
    }
    Variable var = new Variable(id.image, tipoDato.image);
    var.setEstainicializada(inicializada);
    addVariable(var);
  }

  static final public Token typeDATA() throws ParseException {
Token tipoDato;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case INT:
      tipoDato = jj_consume_token(INT);
      break;
    case FLOAT:
      tipoDato = jj_consume_token(FLOAT);
      break;
    case STRING:
      tipoDato = jj_consume_token(STRING);
      break;
    default:
      jj_la1[3] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    {if (true) return tipoDato;}
    throw new Error("Missing return statement in function");
  }

  static final public void value() throws ParseException, SemanticException {
  token id;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case VALUE_INT:
      jj_consume_token(VALUE_INT);
      break;
    case VALUE_FLOAT:
      jj_consume_token(VALUE_FLOAT);
      break;
    case VALUE_STRING:
      jj_consume_token(VALUE_STRING);
      break;
    case ID:
      id = jj_consume_token(ID);
   existeVariable(id.image);
      break;
    default:
      jj_la1[4] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

  static final public void grammarwrite() throws ParseException, SemanticException {
    jj_consume_token(WRITE);
    jj_consume_token(OP);
    value();
    label_2:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case PLUS:
        ;
        break;
      default:
        jj_la1[5] = jj_gen;
        break label_2;
      }
      jj_consume_token(PLUS);
      value();
    }
    jj_consume_token(CP);
  }

  static final public void grammarread() throws ParseException, SemanticException {
                                                 token id;
    jj_consume_token(READ);
    jj_consume_token(OP);
    id = jj_consume_token(ID);
    jj_consume_token(CP);
   existeVariable(id.image);
  }

  static final public void grammarAssignment() throws ParseException, SemanticException {
                                                       token id;
    id = jj_consume_token(ID);
    jj_consume_token(SAME);
    operationSimple();
  existeVariable(id.image);
  }

  static final public void operationSimple() throws ParseException, SemanticException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case ID:
    case VALUE_INT:
    case VALUE_FLOAT:
    case VALUE_STRING:
      value();
      break;
    case OP:
      operationParentesis();
      break;
    default:
      jj_la1[6] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    label_3:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case PLUS:
      case SUBTRACTION:
      case MULTIPLICATION:
      case DIVISION:
      case MODULE:
        ;
        break;
      default:
        jj_la1[7] = jj_gen;
        break label_3;
      }
      operatingArithmetic();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case ID:
      case VALUE_INT:
      case VALUE_FLOAT:
      case VALUE_STRING:
        value();
        break;
      case OP:
        operationParentesis();
        break;
      default:
        jj_la1[8] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
  }

  static final public void operationParentesis() throws ParseException, SemanticException {
    jj_consume_token(OP);
    operationSimple();
    jj_consume_token(CP);
  }

  static final public void operatingArithmetic() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case PLUS:
      jj_consume_token(PLUS);
      break;
    case SUBTRACTION:
      jj_consume_token(SUBTRACTION);
      break;
    case MULTIPLICATION:
      jj_consume_token(MULTIPLICATION);
      break;
    case DIVISION:
      jj_consume_token(DIVISION);
      break;
    case MODULE:
      jj_consume_token(MODULE);
      break;
    default:
      jj_la1[9] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

  static final public void grammarIF() throws ParseException, SemanticException {
    jj_consume_token(IF);
    jj_consume_token(OP);
    condition();
    jj_consume_token(CP);
    jj_consume_token(THEN);
    jj_consume_token(OPK);
    label_4:
    while (true) {
      sentences();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case VAR:
      case WRITE:
      case READ:
      case IF:
      case SELECT:
      case FOR:
      case DOWHILE:
      case WHILE:
      case ID:
        ;
        break;
      default:
        jj_la1[10] = jj_gen;
        break label_4;
      }
    }
    jj_consume_token(CLK);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case ELSE:
      jj_consume_token(ELSE);
      jj_consume_token(OPK);
      label_5:
      while (true) {
        sentences();
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case VAR:
        case WRITE:
        case READ:
        case IF:
        case SELECT:
        case FOR:
        case DOWHILE:
        case WHILE:
        case ID:
          ;
          break;
        default:
          jj_la1[11] = jj_gen;
          break label_5;
        }
      }
      jj_consume_token(CLK);
      break;
    default:
      jj_la1[12] = jj_gen;
      ;
    }
    jj_consume_token(END);
    jj_consume_token(IF);
  }

  static final public void condition() throws ParseException, SemanticException {
    conditionSimple();
    label_6:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case AND:
      case OR:
        ;
        break;
      default:
        jj_la1[13] = jj_gen;
        break label_6;
      }
      opLogical();
      conditionSimple();
    }
  }

  static final public void conditionSimple() throws ParseException, SemanticException {
    value();
    opRelational();
    value();
  }

  static final public void opLogical() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case AND:
      jj_consume_token(AND);
      break;
    case OR:
      jj_consume_token(OR);
      break;
    default:
      jj_la1[14] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

  static final public void opRelational() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case HIGHER:
      jj_consume_token(HIGHER);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case SAME:
        jj_consume_token(SAME);
        break;
      default:
        jj_la1[15] = jj_gen;
        ;
      }
      break;
    case LESS:
      jj_consume_token(LESS);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case SAME:
        jj_consume_token(SAME);
        break;
      default:
        jj_la1[16] = jj_gen;
        ;
      }
      break;
    case SAME:
      jj_consume_token(SAME);
      jj_consume_token(SAME);
      break;
    case NOT:
      jj_consume_token(NOT);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case SAME:
        jj_consume_token(SAME);
        break;
      default:
        jj_la1[17] = jj_gen;
        ;
      }
      break;
    default:
      jj_la1[18] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

  static final public void grammarSelect() throws ParseException, SemanticException {
    jj_consume_token(SELECT);
    jj_consume_token(OP);
    jj_consume_token(ID);
    jj_consume_token(CP);
    label_7:
    while (true) {
      jj_consume_token(CASE);
      jj_consume_token(VALUE_INT);
      jj_consume_token(TP);
      sentences();
      jj_consume_token(BREAK);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case CASE:
        ;
        break;
      default:
        jj_la1[19] = jj_gen;
        break label_7;
      }
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case DEFECT:
      jj_consume_token(DEFECT);
      jj_consume_token(TP);
      sentences();
      jj_consume_token(BREAK);
      break;
    default:
      jj_la1[20] = jj_gen;
      ;
    }
    jj_consume_token(END);
    jj_consume_token(SELECT);
  }

  static final public void grammarFor() throws ParseException, SemanticException {
    jj_consume_token(FOR);
    jj_consume_token(OP);
    jj_consume_token(ID);
    jj_consume_token(SAME);
    jj_consume_token(VALUE_INT);
    jj_consume_token(UNTIL);
    jj_consume_token(SAME);
    jj_consume_token(VALUE_INT);
    jj_consume_token(CP);
    jj_consume_token(OPK);
    label_8:
    while (true) {
      sentences();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case VAR:
      case WRITE:
      case READ:
      case IF:
      case SELECT:
      case FOR:
      case DOWHILE:
      case WHILE:
      case ID:
        ;
        break;
      default:
        jj_la1[21] = jj_gen;
        break label_8;
      }
    }
    jj_consume_token(CLK);
    jj_consume_token(NEXT);
  }

  static final public void grammarWhile() throws ParseException, SemanticException {
    jj_consume_token(WHILE);
    jj_consume_token(OP);
    condition();
    jj_consume_token(CP);
    jj_consume_token(OPK);
    label_9:
    while (true) {
      sentences();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case VAR:
      case WRITE:
      case READ:
      case IF:
      case SELECT:
      case FOR:
      case DOWHILE:
      case WHILE:
      case ID:
        ;
        break;
      default:
        jj_la1[22] = jj_gen;
        break label_9;
      }
    }
    jj_consume_token(CLK);
    jj_consume_token(END);
    jj_consume_token(WHILE);
  }

  static final public void grammarDowhile() throws ParseException, SemanticException {
    jj_consume_token(DOWHILE);
    jj_consume_token(OPK);
    label_10:
    while (true) {
      sentences();
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case VAR:
      case WRITE:
      case READ:
      case IF:
      case SELECT:
      case FOR:
      case DOWHILE:
      case WHILE:
      case ID:
        ;
        break;
      default:
        jj_la1[23] = jj_gen;
        break label_10;
      }
    }
    jj_consume_token(END);
    jj_consume_token(WHILE);
    jj_consume_token(OP);
    condition();
    jj_consume_token(CP);
    jj_consume_token(CLK);
  }

  static private boolean jj_initialized_once = false;
  /** Generated Token Manager. */
  static public ParserTokenManager token_source;
  static SimpleCharStream jj_input_stream;
  /** Current token. */
  static public Token token;
  /** Next token. */
  static public Token jj_nt;
  static private int jj_ntk;
  static private int jj_gen;
  static final private int[] jj_la1 = new int[24];
  static private int[] jj_la1_0;
  static private int[] jj_la1_1;
  static {
      jj_la1_init_0();
      jj_la1_init_1();
   }
   private static void jj_la1_init_0() {
      jj_la1_0 = new int[] {0x3226c00,0x3226c00,0x0,0x380,0x0,0x4000000,0x0,0x7c000000,0x0,0x7c000000,0x3226c00,0x3226c00,0x8000,0x0,0x0,0x0,0x0,0x0,0x80000000,0x40000,0x80000,0x3226c00,0x3226c00,0x3226c00,};
   }
   private static void jj_la1_init_1() {
      jj_la1_1 = new int[] {0x1000,0x1000,0x2,0x0,0xf000,0x0,0xf020,0x0,0xf020,0x0,0x1000,0x1000,0x0,0xc,0xc,0x2,0x2,0x2,0x13,0x0,0x0,0x1000,0x1000,0x1000,};
   }

  /** Constructor with InputStream. */
  public Parser(java.io.InputStream stream) {
     this(stream, null);
  }
  /** Constructor with InputStream and supplied encoding */
  public Parser(java.io.InputStream stream, String encoding) {
    if (jj_initialized_once) {
      System.out.println("ERROR: Second call to constructor of static parser.  ");
      System.out.println("       You must either use ReInit() or set the JavaCC option STATIC to false");
      System.out.println("       during parser generation.");
      throw new Error();
    }
    jj_initialized_once = true;
    try { jj_input_stream = new SimpleCharStream(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
    token_source = new ParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  static public void ReInit(java.io.InputStream stream) {
     ReInit(stream, null);
  }
  /** Reinitialise. */
  static public void ReInit(java.io.InputStream stream, String encoding) {
    try { jj_input_stream.ReInit(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
    token_source.ReInit(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  /** Constructor. */
  public Parser(java.io.Reader stream) {
    if (jj_initialized_once) {
      System.out.println("ERROR: Second call to constructor of static parser. ");
      System.out.println("       You must either use ReInit() or set the JavaCC option STATIC to false");
      System.out.println("       during parser generation.");
      throw new Error();
    }
    jj_initialized_once = true;
    jj_input_stream = new SimpleCharStream(stream, 1, 1);
    token_source = new ParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  static public void ReInit(java.io.Reader stream) {
    jj_input_stream.ReInit(stream, 1, 1);
    token_source.ReInit(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  /** Constructor with generated Token Manager. */
  public Parser(ParserTokenManager tm) {
    if (jj_initialized_once) {
      System.out.println("ERROR: Second call to constructor of static parser. ");
      System.out.println("       You must either use ReInit() or set the JavaCC option STATIC to false");
      System.out.println("       during parser generation.");
      throw new Error();
    }
    jj_initialized_once = true;
    token_source = tm;
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  public void ReInit(ParserTokenManager tm) {
    token_source = tm;
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 24; i++) jj_la1[i] = -1;
  }

  static private Token jj_consume_token(int kind) throws ParseException {
    Token oldToken;
    if ((oldToken = token).next != null) token = token.next;
    else token = token.next = token_source.getNextToken();
    jj_ntk = -1;
    if (token.kind == kind) {
      jj_gen++;
      return token;
    }
    token = oldToken;
    jj_kind = kind;
    throw generateParseException();
  }


/** Get the next Token. */
  static final public Token getNextToken() {
    if (token.next != null) token = token.next;
    else token = token.next = token_source.getNextToken();
    jj_ntk = -1;
    jj_gen++;
    return token;
  }

/** Get the specific Token. */
  static final public Token getToken(int index) {
    Token t = token;
    for (int i = 0; i < index; i++) {
      if (t.next != null) t = t.next;
      else t = t.next = token_source.getNextToken();
    }
    return t;
  }

  static private int jj_ntk() {
    if ((jj_nt=token.next) == null)
      return (jj_ntk = (token.next=token_source.getNextToken()).kind);
    else
      return (jj_ntk = jj_nt.kind);
  }

  static private java.util.List<int[]> jj_expentries = new java.util.ArrayList<int[]>();
  static private int[] jj_expentry;
  static private int jj_kind = -1;

  /** Generate ParseException. */
  static public ParseException generateParseException() {
    jj_expentries.clear();
    boolean[] la1tokens = new boolean[48];
    if (jj_kind >= 0) {
      la1tokens[jj_kind] = true;
      jj_kind = -1;
    }
    for (int i = 0; i < 24; i++) {
      if (jj_la1[i] == jj_gen) {
        for (int j = 0; j < 32; j++) {
          if ((jj_la1_0[i] & (1<<j)) != 0) {
            la1tokens[j] = true;
          }
          if ((jj_la1_1[i] & (1<<j)) != 0) {
            la1tokens[32+j] = true;
          }
        }
      }
    }
    for (int i = 0; i < 48; i++) {
      if (la1tokens[i]) {
        jj_expentry = new int[1];
        jj_expentry[0] = i;
        jj_expentries.add(jj_expentry);
      }
    }
    int[][] exptokseq = new int[jj_expentries.size()][];
    for (int i = 0; i < jj_expentries.size(); i++) {
      exptokseq[i] = jj_expentries.get(i);
    }
    return new ParseException(token, exptokseq, tokenImage);
  }

  /** Enable tracing. */
  static final public void enable_tracing() {
  }

  /** Disable tracing. */
  static final public void disable_tracing() {
  }

}
